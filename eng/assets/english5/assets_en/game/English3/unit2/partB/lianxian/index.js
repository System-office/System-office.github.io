...
function mouseStart(event, id) {
  ...
  if (imgArr.indexOf(id) == -1) {
    ...
    mouse_start_x = event.target.offsetLeft + (event.target.offsetWidth /1.6);
    mosue_start_y = event.target.offsetTop + (event.target.offsetHeight / 1.12)
    mouse_pos_x = mouse_start_x;
    mouse_pos_y = mosue_start_y;
...
function mouse_click(startId, event) {
  ...
  if (mouse_start_x && mosue_start_y) {
    ...
    if (endRight == currentRight) {
      ...
      mouse_pos_x = event.target.offsetLeft + (event.target.offsetWidth / 2.2);
      mouse_pos_y = event.target.offsetTop + (event.target.offsetHeight / 15);
      draw()
      if (startId == 'dot1') {       //这里给每个id匹配一个img标签
        ...
        image7.src = canvas.toDataURL("image/png");
        image7.setAttribute('data-a', startId)
      }else if (startId == 'dot8') {
        var image8 = document.getElementById('img8')
        image8.src = canvas.toDataURL("image/png");
        image8.setAttribute('data-a', startId)
      }
    } else {
      // 错误
...
function draw() {
  ...
  ctx.lineWidth = line;
  ctx.setLineDash([2,2]);
  ctx.strokeStyle = "#c24030";
  ctx.fillStyle = "c24030";
  //Draw line
  ctx.beginPath();
