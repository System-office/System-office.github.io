  ...
  numberList[i].addEventListener('mouseup', function(e) {
    if (e.target.parentNode.nodeName === 'DIV') {
      ...
      for (var i = 0; i < scope.length; i++) {
        // 判断是否拖入方框
        if ((x1 >= scope[0].x1 && x2 <= scope[0].x2 && y1 >= scope[0].y1 && y2 <= scope[0].y2)) {
          
          // 打断前一次播放
          // 是否正确
          if (e.target.getAttribute('data-flag') == data[listIndex]) {
...
// 播放音频
var soundDom = document.createElement('audio')
var soundIndex = 0
function playBgSound() {
  soundDom.src = sound[soundIndex]
  soundDom.play()
  soundDom.oncanplay = function () {
    ...
    }, parseInt(soundDom.duration * 1000))
  }
}
